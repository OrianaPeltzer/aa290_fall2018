...
def graph(state_that_has_just_been_trained_on,same_in_grid, start_state, goal_state, my_graph, pct_successful,ep_mean_rews,k0=0,**kwargs):
    ...
    next_path,next_path_in_grid = my_graph.get_shortest_path(start_state,goal_state) #This outputs a list of tuples

    if len(next_path) <= 2:
        return True, my_graph

    np2 = next_path_in_grid[-2]
...
def graph_fmt(state_that_has_just_been_trained_on, same_in_grid, start_state, goal_state, my_graph, pct_successful,ep_mean_rews,environment,k0=300,**kwargs):

    gamma=0.5
    M = 10000 #Very big (failure)

    cost_to_go = float(environment.control_cost)

    print("Percentage successful: ", str(pct_successful*100))

    success_after_training = float(ep_mean_rews[-1])/1000. #+ 0.5 #Map the reward to a success score btw -1 and 1
    if np.mean(ep_mean_rews) <= 0.1:
        real_cost = M
    else:
        real_cost = cost_to_go*success_after_training/(2*(gamma-1)) + cost_to_go*(gamma-2)/(2*(gamma-1)) #Cost once ppo has trained on this node

    my_graph.reward_history += [ep_mean_rews[-1]]

    # Add edge to graph linking goal state and trained node
    my_graph.assign_to_graph_in_grid(same_in_grid,my_graph.transform_to_grid(goal_state),real_cost) #This also updates/overwrites total_fmt costs.

    print("We assigned cost ", -real_cost, "to the edges in the grid ", same_in_grid, "and the goal.")

    my_graph.fmt_total_g[tuple(same_in_grid)][tuple(my_graph.transform_to_grid(goal_state))] = real_cost

    #Tell the graph that this node is explored
    my_graph.explored_nodes += [same_in_grid]
    ...
    if np.isin(explored_nodes,np2).all(axis=1).any():
        ...
        my_graph.delete_edge_in_grid(np3,np2)
        my_graph.delete_edge_fmt_total_in_grid(np3,np2)
        try:
        ...
        next_point_to_train_on = list(next_path[-3]) #Need to convert tuples to lists
        next_point_to_train_on_in_grid = list(np3)
    else:
        print("Node ", np2, "is new! Let's delete its link to the goal and explore it")
        my_graph.delete_edge_in_grid(np2, my_graph.transform_to_grid(goal_state)) #delete from graph.g, the graph to search from
        my_graph.delete_edge_fmt_total_in_grid(np2, my_graph.transform_to_grid(goal_state)) #Also delete from stored values in big storage graph
        next_point_to_train_on = list(next_path[-2])
        next_point_to_train_on_in_grid = list(np2)
    ...
    # return a new starting point in real coordinates
    return next_point_to_train_on, next_point_to_train_on_in_grid, my_graph

def graph_fmt_car(state_that_has_just_been_trained_on, same_in_grid, start_state, goal_state, my_graph, pct_successful,ep_mean_rews,step_size,environment,k0=300,index_in_path=None,**kwargs):
    ...
    if np.isin(explored_nodes,np2).all(axis=1).any():
        ...
        next_point_to_train_on = list(next_path[-3]) #Need to convert tuples to lists
        np_d = (next_point_to_train_on[0],next_point_to_train_on[1],next_point_to_train_on[2])
        point_to_go_from_it = list(next_path[-2])
        next_point_to_train_on_in_grid = list(np3)
    else:
        print("Node ", np2, "is new! Let's delete its link to the goal and explore it")
        my_graph.delete_edge_in_grid(np2, my_graph.transform_to_grid(goal_state)) #delete from graph.g, the graph to search from
        my_graph.delete_edge_fmt_total_in_grid(np2, my_graph.transform_to_grid(goal_state)) #Also delete from stored values in big storage graph
        next_point_to_train_on = list(next_path[-2])
        point_to_go_from_it = list(next_path[-1])
        pg_d = (point_to_go_from_it[0],point_to_go_from_it[1],point_to_go_from_it[2])
        next_point_to_train_on_in_grid = list(np2)
    ...
    current_path_dubins = dubins.shortest_path(np_d,pg_d,environment.turning_radius)
    current_path = current_path_dubins.sample_many(step_size)
    ...
    # return a new starting point in real coordinates
    return next_point_to_train_on, next_point_to_train_on_in_grid, my_graph, current_path